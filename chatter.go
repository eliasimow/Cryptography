...
import (
	//un-comment for helpers like bytes.equal
	"encoding/binary"
	"errors"
...
func (c *Chatter) ReturnHandshake(partnerIdentity,
	...
	partOne := DHCombine(partnerIdentity, &c.Sessions[*partnerIdentity].MyDHRatchet.PrivateKey)
	partTwo := DHCombine(partnerEphemeral, &c.Identity.PrivateKey)
	partThree := DHCombine(partnerEphemeral, &c.Sessions[*partnerIdentity].MyDHRatchet.PrivateKey)

	c.Sessions[*partnerIdentity].RootChain = CombineKeys(partOne, partTwo, partThree)
	c.Sessions[*partnerIdentity].ReceiveChain = c.Sessions[*partnerIdentity].RootChain
	c.Sessions[*partnerIdentity].SendChain = c.Sessions[*partnerIdentity].ReceiveChain
...
func (c *Chatter) FinalizeHandshake(partnerIdentity,
	...
	partOne := DHCombine(partnerEphemeral, &c.Identity.PrivateKey)
	partTwo := DHCombine(partnerIdentity, &c.Sessions[*partnerIdentity].MyDHRatchet.PrivateKey)
	partThree := DHCombine(partnerEphemeral, &c.Sessions[*partnerIdentity].MyDHRatchet.PrivateKey)
...
func (c *Chatter) SendMessage(partnerIdentity *PublicKey,
	...
	if c.Sessions[*partnerIdentity].shouldUpdate == true {
		...
		c.Sessions[*partnerIdentity].RootChain = CombineKeys(c.Sessions[*partnerIdentity].RootChain, newDH)
		c.Sessions[*partnerIdentity].SendChain = c.Sessions[*partnerIdentity].RootChain
		//c.Sessions[*partnerIdentity].SentThisChainCounter = 0
		c.Sessions[*partnerIdentity].shouldUpdate = false

	}

	c.Sessions[*partnerIdentity].SendChain = c.Sessions[*partnerIdentity].SendChain.DeriveKey(CHAIN_LABEL)
	//Recursively derie through to current chain

	chainKey := c.Sessions[*partnerIdentity].SendChain.DeriveKey(KEY_LABEL)
	...
	message.Ciphertext = chainKey.AuthenticatedEncrypt(plaintext, message.EncodeAdditionalData(), message.IV)
...
func (c *Chatter) ReceiveMessage(message *Message) (string, error) {
	...
	// store so later can revert after check for tampered data:

	storedRootChain := c.Sessions[*message.Sender].RootChain
	storedReceiveChain := c.Sessions[*message.Sender].ReceiveChain
	storedDHChain := c.Sessions[*message.Sender].PartnerDHRatchet
	storedArr := c.Sessions[*message.Sender].CachedReceiveKeys
	storedUpdateState := c.Sessions[*message.Sender].shouldUpdate
	storedCacheFilledTo := c.Sessions[*message.Sender].cacheFilledTo
	c.Sessions[*message.Sender].ReceiveCounter++
	ReceiveChainKey := c.Sessions[*message.Sender].ReceiveChain.DeriveKey(KEY_LABEL)

	if message.Counter >= c.Sessions[*message.Sender].cacheFilledTo {
		// early message:

		//fmt.Println("early message")
		if message.NextDHRatchet == c.Sessions[*message.Sender].PartnerDHRatchet {
			//Same DH root:

			for i := message.LastUpdate; i <= message.Counter; i++ {

				if c.Sessions[*message.Sender].cacheFilledTo < i {
					//Put into memory if i is in this DHRatchet range
					c.Sessions[*message.Sender].ReceiveChain = c.Sessions[*message.Sender].ReceiveChain.DeriveKey(CHAIN_LABEL)
					c.Sessions[*message.Sender].cacheFilledTo = i
					messageKey := c.Sessions[*message.Sender].ReceiveChain.DeriveKey(KEY_LABEL)
					c.Sessions[*message.Sender].CachedReceiveKeys[i] = messageKey
				}
			}

			//Then return with message key of this early index
			ReceiveChainKey = c.Sessions[*message.Sender].CachedReceiveKeys[message.Counter]
			value, nul := ReceiveChainKey.AuthenticatedDecrypt(message.Ciphertext, message.EncodeAdditionalData(), message.IV)
			calcCipher := ReceiveChainKey.AuthenticatedEncrypt(value, message.EncodeAdditionalData(), message.IV)
			if !ByteEqual(calcCipher, message.Ciphertext) {
				c.Sessions[*message.Sender].RootChain = storedRootChain
				c.Sessions[*message.Sender].ReceiveChain = storedReceiveChain
				c.Sessions[*message.Sender].CachedReceiveKeys = storedArr
				c.Sessions[*message.Sender].shouldUpdate = storedUpdateState
				c.Sessions[*message.Sender].cacheFilledTo = storedCacheFilledTo
				c.Sessions[*message.Sender].ReceiveCounter--
				c.Sessions[*message.Sender].PartnerDHRatchet = storedDHChain
				return "", errors.New("Failed to authenticate")
			}
			c.Sessions[*message.Sender].CachedReceiveKeys[message.Counter].Zeroize()
			ReceiveChainKey.Zeroize()
			return value, nul
		}
		//Different DH Root:
		c.Sessions[*message.Sender].shouldUpdate = true
		...
		newKey := DHCombine(c.Sessions[*message.Sender].PartnerDHRatchet, &c.Sessions[*message.Sender].MyDHRatchet.PrivateKey)
		...
		//fmt.Println("undestood shared root is ", c.Sessions[*message.Sender].ReceiveChain)
	...
	//Late message:
	//	fmt.Println("late message")
	ReceiveChainKey = c.Sessions[*message.Sender].CachedReceiveKeys[message.Counter]

	if ReceiveChainKey == nil {
		//fmt.Println("error reached, with counter: ", message.Counter, "last update ", c.Sessions[*message.Sender].LastUpdate, "receive counter ", c.Sessions[*message.Sender].ReceiveCounter)
		return "", nil
	}
	value, nul := ReceiveChainKey.AuthenticatedDecrypt(message.Ciphertext, message.EncodeAdditionalData(), message.IV)
